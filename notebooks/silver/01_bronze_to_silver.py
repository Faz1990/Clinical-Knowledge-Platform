...
dbutils.widgets.text("quarantine_table", "clinical_platform.silver.guidelines_quarantine","Quarantine table")
dbutils.widgets.text("manifest_table",   "clinical_platform.bronze.guidelines_manifest",  "Manifest UC table")
...
df_checked = df_new.withColumn(
    ...
    validate_contract(
        F.col("source_url"), F.col("retrieved_at"), F.col("content_hash"),
        F.col("guideline_version"), F.col("pipeline_run_id"), F.col("ingest_timestamp"),
        F.col("length"), F.col("guideline_id"),
    ),
...
df_structural_invalid = (
    ...
    .select(
        "source_url", "retrieved_at", "content_hash", "guideline_version",
        "pipeline_run_id", "ingest_timestamp", "guideline_id",
        "content", "length", "quarantine_reason", "quarantine_timestamp",
    )
)
...
df_scope_checked = df_joined.withColumn(
    ...
    F.when(F.col("expected_clinical_area").isNull(), F.array(F.lit("MISSING_MANIFEST_ENTRY")))
     .when(~F.col("expected_clinical_area").isin(list(ACCEPTED_CLINICAL_AREAS)), F.array(F.lit("INVALID_CLINICAL_AREA")))
...
df_parsed = (
    df_parsed_raw
    .withColumn("parsed_text", F.col("_pdf.parsed_text"))
    ...
    .drop("_pdf", "content", "structural_violations", "passes_structural",
          "scope_violations", "passes_scope", "expected_clinical_area")
)
...
df_silver_ready = (
    ...
    .select(
        "source_url", "retrieved_at", "content_hash", "guideline_version",
        "pipeline_run_id", "ingest_timestamp", "guideline_id",
        "title", "clinical_area", "nice_url", "publication_date",
        "parsed_text", "page_count", "char_count", "word_count",
        "silver_processed_at",
    )
)
