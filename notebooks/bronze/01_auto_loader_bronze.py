...
dbutils.widgets.text("schema_location",  "",  "Auto Loader schema/checkpoint path (abfss://autoloader-schema@...)")
dbutils.widgets.text("bronze_table",     "clinical_platform.bronze.guidelines_raw", "Bronze Delta table")
...
df_bronze = (
    ...
    .withColumn("guideline_id",
                F.regexp_extract(F.col("path"), r"/([A-Z]+\d+)_", 1))
    .select(
        "source_url",
        "retrieved_at",
        "content_hash",
